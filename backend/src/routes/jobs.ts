...
router.get('/search', optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    const jobsResult = await jobBoardService.searchAllJobs(
      query as string,
      location as string,
      limitNum
    );
    ...
    const uniqueJobs = allJobs.filter((job, index, self) => 
      index === self.findIndex(j => j.external_id === job.external_id && j.source === job.source)
    );
...
router.get('/:id/skills-match', authenticate, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    try {
      ...
      const skillsResult = await client.query(
        'SELECT skill, is_required FROM job_skills WHERE job_id = $1',
        [id]
      );
      ...
      const coursesResult = await client.query(
        `SELECT c.* FROM courses c
         ...
         WHERE ap.user_id = $1 AND pc.status = 'completed'`,
      ...
      coursesResult.rows.forEach(course => {
        if (course.code.includes('CS')) {
          ...
          if (course.name.includes('Java')) userSkills.add('Java');
          if (course.name.includes('Python')) userSkills.add('Python');
          if (course.name.includes('Algorithm')) userSkills.add('Algorithms');
          if (course.name.includes('Data Structure')) userSkills.add('Data Structures');
        }
      });

      const matchedSkills = jobSkills.filter(jobSkill => 
        Array.from(userSkills).some(userSkill => 
          userSkill.toLowerCase().includes(jobSkill.skill.toLowerCase()) ||
          jobSkill.skill.toLowerCase().includes(userSkill.toLowerCase())
        )
      );

      const missingSkills = jobSkills.filter(jobSkill => 
        !matchedSkills.some(matched => matched.skill === jobSkill.skill)
      );
