...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        logger_1.logger.error('Error fetching courses:', error);
        res.status(500).json({
            success: false,
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const client = await database_1.pool.connect();
        const result = await client.query(`SELECT c.*, d.name as department_name, s.name as school_name
       FROM courses c
       LEFT JOIN departments d ON c.department_id = d.id
       LEFT JOIN schools s ON c.school_id = s.id
       WHERE c.id = $1`, [id]);
        ...
        // Get prerequisites
        const prereqResult = await client.query(`SELECT c.* FROM courses c
       ...
       WHERE cp.course_id = $1`, [id]);
        ...
        logger_1.logger.error('Error fetching course:', error);
        res.status(500).json({
            success: false,
            error: 'Internal server error'
        });
