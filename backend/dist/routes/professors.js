...
router.get('/', async (req, res) => {
    try {
        const { university = 'Purdue University', department, search, page = 1, limit = 20 } = req.query;
        ...
        LIMIT $${paramCount + 1} OFFSET $${paramCount + 2}`, [...params, limitNum, offset]);
            // Get total count
            const countResult = await client.query(`SELECT COUNT(*) FROM professors ${whereClause}`, params);
            ...
            if (search && professors.length < 5) {
                try {
                    const rmpResults = await professor_1.professorService.searchProfessors(search, university);
                    // Add external results that aren't already in our DB
                    rmpResults.professors.forEach(rmpProf => {
                        const exists = professors.find(p => p.name.toLowerCase().includes(rmpProf.name.toLowerCase()) ||
                            p.rmp_id === rmpProf.id);
                        if (!exists) {
                            professors.push({
                    ...
                    logger_1.logger.warn('Failed to fetch from RMP:', error);
                }
            }
            res.json({
        ...
        logger_1.logger.error('Error fetching professors:', error);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (id.startsWith('external-')) {
            ...
            const professorData = await professor_1.professorService.getProfessorById(rmpId);
            if (!professorData) {
        ...
        logger_1.logger.error('Error fetching professor:', error);
...
router.get('/search/:name', async (req, res) => {
    try {
        ...
        const [localResults, externalResults] = await Promise.allSettled([
            ...
            ORDER BY rating DESC NULLS LAST`, [`%${name}%`, `%${university}%`]);
                    return result.rows;
                }
                finally {
                    client.release();
            ...
            // External search
            professor_1.professorService.searchProfessors(name, university)
        ]);
        const professors = [];
        // Add local results
        ...
        if (externalResults.status === 'fulfilled') {
            externalResults.value.professors.forEach(extProf => {
                const exists = professors.find(p => p.name.toLowerCase().includes(extProf.name.toLowerCase()) ||
                    p.rmp_id === extProf.id);
                if (!exists) {
                    professors.push({
