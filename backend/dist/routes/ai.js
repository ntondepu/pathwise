...
router.post('/resume-help', auth_1.authenticate, async (req, res) => {
    try {
...
Format as numbered list.`;
        ...
        if (userResult.rows.length > 0) {
            await client.query(`INSERT INTO career_advice (user_id, type, prompt, response)
         VALUES ($1, 'resume', $2, $3)`, [userResult.rows[0].id, prompt, response]);
        }
        client.release();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error generating resume help:', error);
        res.status(500).json({
            success: false,
...
router.post('/interview-prep', auth_1.authenticate, async (req, res) => {
    try {
        const { courses, skills, jobTitle } = req.body;
        const firebaseUid = req.user.uid;
...
Format each question clearly with the question type in parentheses.`;
        ...
        if (userResult.rows.length > 0) {
            await client.query(`INSERT INTO career_advice (user_id, type, prompt, response)
         VALUES ($1, 'interview', $2, $3)`, [userResult.rows[0].id, prompt, response]);
        }
        client.release();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error generating interview prep:', error);
        res.status(500).json({
            success: false,
...
router.post('/certifications', auth_1.authenticate, async (req, res) => {
    try {
...
Format as a numbered list with clear sections for each certification.`;
        ...
        if (userResult.rows.length > 0) {
            await client.query(`INSERT INTO career_advice (user_id, type, prompt, response)
         VALUES ($1, 'certification', $2, $3)`, [userResult.rows[0].id, prompt, response]);
        }
        client.release();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error generating certification recommendations:', error);
        res.status(500).json({
            success: false,
...
router.get('/history', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await client.query(`SELECT * FROM career_advice 
       WHERE user_id = $1 
       ORDER BY created_at DESC 
       LIMIT 50`, [userResult.rows[0].id]);
        client.release();
        res.json({
            success: true,
            data: result.rows
        ...
        logger_1.logger.error('Error fetching AI history:', error);
        res.status(500).json({
            success: false,
